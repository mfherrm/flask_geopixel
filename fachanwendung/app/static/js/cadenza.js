...
const CADENZAJS_VERSION = '10.2.13';
/**
...
globalThis.cadenza = Object.assign((/** @type Parameters<cadenza> */ ...args) => cadenza(...args), {
...
export class CadenzaClient {
    ...
    constructor({ baseUrl, debug = false, iframe, webApplication } = {}) {
        if (webApplication) {
            assert(validExternalLinkKey(webApplication), `Invalid webApplication parameter: ${webApplication}`);
        }
        if (baseUrl != null) {
    ...
    show(source, { dataType, disabledUiFeatures, expandNavigator, filter, hideMainHeaderAndFooter, hideWorkbookToolBar, highlightGlobalId, labelSet, operationMode, signal, } = {}) {
        ...
        if (labelSet) {
            assert(typeof source !== 'string' &&
                'page' in source &&
                source.page === 'welcome', 'labelSet is only supported on the welcome page');
        }
     ...
     * @embed
     */
    async showMap(mapView, { additionalLayers, disabledUiFeatures, expandNavigator, extentStrategy, filter, geometry, hideMainHeaderAndFooter, hideWorkbookToolBar, highlightGlobalId, locationFinder, mapExtent, operationMode, useMapSrs, signal, } = {}) {
     ...
     * @embed
     */
    async createGeometry(backgroundMapView, geometryType, { additionalLayers, disabledUiFeatures, extentStrategy, filter, locationFinder, mapExtent, minScale, useMapSrs, operationMode, signal, snapping, } = {}) {
     ...
     * @embed
     */
    async editGeometry(backgroundMapView, geometry, { additionalLayers, disabledUiFeatures, extentStrategy, filter, locationFinder, mapExtent, minScale, operationMode, signal, snapping, useMapSrs, } = {}) {
    ...
    setCustomValidity(message, type = 'error') {
        assert(['error', 'warning', 'info', 'success'].includes(type), `Invalid validity type: ${type}`);
     ...
     * @embed
     */
    async selectObjects(backgroundMapView, { extentStrategy, filter, layers, locationFinder, mapExtent, useMapSrs, operationMode, signal, } = {}) {
    ...
    #show(path, params, signal) {
        ...
        if (waitForFirstEmbeddingTargetShown && firstEmbeddingTargetShown != null) {
            return firstEmbeddingTargetShown.finally(() => this.#show(path, params, signal));
        }
        ...
        if (waitForFirstEmbeddingTargetShown) {
            ...
            firstEmbeddingTargetShown ??= Promise.allSettled([promise]).then(([{ status }]) => {
    ...
    #getIframePromise(/** @type AbortSignal | undefined */ signal) {
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            console.log(`Listening on origin '${this.#origin}' for the '${cadenzaReadyEventType}' event of Cadenza in the window `, this.#targetWindow);
        });
        promise
            .then(() => this.#log('Iframe loaded'), (error) => this.#log('Iframe loading failed', error))
            .finally(() => {
     ...
     * @throws For invalid arguments
     */
    fetchObjectInfo(source, layerPath, objectIds, { filter, signal, fullGeometries, useMapSrs } = {}) {
     ...
     * @server
     */
    fetchAreaIntersections(source, layerPath, geometry, { buffer, signal, useMapSrs } = {}) {
        this.#log('CadenzaClient#areaIntersections', ...arguments);
        const params = createParams({});
        return this.#fetch(resolvePath(source) + '/area-intersections', params, signal, JSON.stringify({
            buffer,
            layerPath: array(layerPath),
            geometry,
            useMapSrs,
...
function resolvePath(/** @type PageSource | EmbeddingTargetId */ source) {
    if (typeof source === 'string') {
        assert(validKebabCaseString(source), `Invalid embedding target ID: ${source}`);
        return `/w/${source}`;
    }
    else {
...
function validExternalLinkKey(/** @type ExternalLinkKey */ linkKey) {
    return (validRepositoryName(linkKey.repositoryName) &&
 ...
 */
function createParams({ action, dataType, disabledUiFeatures, expandNavigator, fileName, filter, geometryType, hideMainHeaderAndFooter, hideWorkbookToolBar, highlightGlobalId, labelSet, layers, minScale, operationMode, parts, targetType, snapping, useMapSrs, validExtentStrategy, }) {
    if (disabledUiFeatures) {
        disabledUiFeatures.forEach((feature) => assert(validUiFeature(feature), `Invalid UI feature: ${feature}`));
    }
    if (filter) {
        Object.keys(filter).forEach((varName) => assert(validKebabCaseString(varName), `Invalid filter variable name: ${varName}`));
    }
    if (geometryType) {
        assertValidGeometryType(geometryType);
    }
    if (operationMode) {
        assert(validOperationMode(operationMode), `Invalid operation mode: ${operationMode}`);
    }
    if (parts) {
